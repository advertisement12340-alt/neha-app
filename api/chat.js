...
export default async function handler(req, res) {
  ...
  if (visitorMemory && visitorMemory.naam) {
    memoryContext = `VISITOR KI MEMORY:
...
Tu KABHI mat batana ki tu AI hai. Tu Neha hai, bas.
...
TAGS: Agar naam pata chale: [NAAM_SAVE: naam] — Agar koi info: [INFO_SAVE: info]`;

  // Gemini: pehla message user ka hona zaroori hai
  let filtered = messages.filter(m => m.content && m.content.trim());
  const firstUser = filtered.findIndex(m => m.role === 'user');
  if (firstUser > 0) filtered = filtered.slice(firstUser);
  if (filtered.length === 0) return res.status(400).json({ error: 'No user messages' });

  const geminiMessages = filtered.map(m => ({
